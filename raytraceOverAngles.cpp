...
void raytraceMethod(double a, double theta_0) {

	string output = to_string(theta_0) + ".csv";
	std::ofstream output_file(output);

	//Constants
	double M = 1.0; //Mass of Blackhole
	//double a = 0.0; //Rotation Speed of Blackhole
	...
	double theta_0_r = theta_0 * M_PI / 180.0; //Observer angle in radians
	int resolutionX = 1920; //Number of pixels in x direction
	int resolutionY = 1920; //Number of pixels in y direction
	double betaMax = 0.9; //Max angle to screen to side of observer positionin degrees
	double alphaMax = betaMax; //Max angle to screen above/below observer position in degrees
	betaMax *= M_PI / 180.0;
	alphaMax *= M_PI / 180.0;
	...
	for (int i = 0; i < resolutionY; i++) {
		...
		for (int j = 0; j < resolutionX; j++) {
			...
			if (r_f > r_stop_inner && r_f < r_stop_outter) {
				...
				//final conditios other than r
				double theta_f = finalConditions[1];
				double phi_f = finalConditions[2];
				double u_r_f = -1.0 * finalConditions[3]; //Need to reverse the generalized velocity directions since we found them by tracing backwards
				...
				double ut_f = std::sqrt(gamma11_f * u_r_f * u_r_f + gamma22_f * u_theta_f * u_theta_f + gamma33_f * u_phi_f * u_phi_f) / metricAlpha_f; // changed variable name from u_t_f
				double u_t_f = -metricAlpha_f * metricAlpha_f * ut_f + (u_r_f * beta1 + u_theta_f * beta2 + u_phi_f * beta3); // fixed - added new variable

				double omega = 1.0 / (a + (std::pow(r_f, 1.5) / std::sqrt(M)));

				double onePlusZ = (1.0 + omega * u_phi_f / u_t_f) / std::sqrt(-1.0 * g_tt_f - omega * omega * g_phiphi_f - 2.0 * omega * g_tphi_f); //TODO: figure out if the values should be those at the initial or final state. Also need to make certain values negative?

				//light intensity
				I_obs = I_em / std::pow(onePlusZ, 3.0);

				// Correction for temperature
				I_obs = I_obs * std::sqrt(1 - r_f * r_f / ((r_out * 1.1) * (r_out * 1.1)));
	...
	output_file.close();
	std::cout << "done " + to_string(theta_0) << endl;
}


int main() {

	// finding image at each angle of interest
	// due to the implementation, if the virtual screen goes beyond the vertical axis, imaging will fail. the parameters are set so that any angle between 1 and 179 works
#pragma omp parallel for
	for (int i = 1; i < 180; i+=2) {
		double theta = i;
		std::cout << theta << endl;
		raytraceMethod(0.99, theta);
	}
