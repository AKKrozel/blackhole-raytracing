...
public:
  ...
  std::vector<double> integrate(const F &f, const StopCondition &stop, double h,
    ...
    // Clear the arrays so that separate calls don't interfere with eachother
    xs.clear();
    result.clear();
    ...
    // Numerical integration until stop condition is reached
    while (stop(x, y) == false) {
      ...
      // Accept the step if error is below 1, otherwise reject it and do not move forward
      if (err < 1.0) {
        x += h;
        xs.push_back(x);
        result.push_back(y);
      ...
      // Adjust step size h depending on error 
      double err_alpha = 0.7 / 5.0;
      double err_beta = 0.4 / 5.0;
      h = std::max(hmin, 0.9 * h * std::pow(err, -err_alpha) * std::pow(err_prev, err_beta));
      h = std::min(hmax, h);
      err_prev = err;
  ...
  std::vector<double> step(const F& f, double h, double x, const std::vector<double> &y) {
    ...
    // Estimate y_err for each y in the vector using the difference between y1 and y2
    for (int i = 0; i < n_eq; i++) {
